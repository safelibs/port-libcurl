...
static CURLcode run_case(CURLSH *share, const char *host, const char *entry,
                         long dns_timeout, struct trace_data *trace)
{
  CURL *curl = NULL;
  ...
  if(res != CURLE_OK)
    goto cleanup;
  res = curl_easy_setopt(curl, CURLOPT_DNS_CACHE_TIMEOUT, dns_timeout);
  if(res != CURLE_OK)
    goto cleanup;
...
int test(char *URL)
{
  ...
  res = run_case(share, "test.com",
                 "TEST.com:1807:,,127.0.0.1,,,127.0.0.2,,,,",
                 60L, &trace);
  if(res != CURLE_COULDNT_CONNECT || !trace.saw_ipv4 ||
     strcmp(trace.first_ip, "127.0.0.1")) {
    fprintf(stderr, "parser case failed, got '%s' and %d\n",
            trace.saw_ipv4 ? trace.first_ip : "(none)", (int)res);
    res = TEST_ERR_MAJOR_BAD;
    goto test_cleanup;
  }

  res = run_case(share, "temp.example",
                 "+temp.example:1807:127.0.0.1",
                 DNS_TIMEOUT, &trace);
  if(res != CURLE_COULDNT_CONNECT || !trace.saw_ipv4 ||
     strcmp(trace.first_ip, "127.0.0.1")) {
    fprintf(stderr, "temporary entry case failed, got '%s' and %d\n",
            trace.saw_ipv4 ? trace.first_ip : "(none)", (int)res);
    res = TEST_ERR_MAJOR_BAD;
    goto test_cleanup;
  }

  sleep((unsigned int)DNS_TIMEOUT + 1U);

  res = run_case(share, "temp.example", NULL, DNS_TIMEOUT, &trace);
  if(res != CURLE_COULDNT_RESOLVE_HOST || trace.saw_ipv4) {
    fprintf(stderr, "temporary entry did not expire as expected\n");
    res = TEST_ERR_MAJOR_BAD;
    goto test_cleanup;
  }
