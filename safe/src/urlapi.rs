...
fn url_error_string(code: CURLUcode) -> *const c_char {
    match code {
        CURLUE_OK => cstr!("No error").as_ptr(),
        CURLUE_BAD_HANDLE => cstr!("Bad URL handle").as_ptr(),
        CURLUE_BAD_PARTPOINTER => cstr!("Bad URL part pointer").as_ptr(),
        CURLUE_MALFORMED_INPUT => cstr!("Malformed input").as_ptr(),
        CURLUE_BAD_PORT_NUMBER => cstr!("Bad port number").as_ptr(),
        CURLUE_OUT_OF_MEMORY => cstr!("Out of memory").as_ptr(),
        CURLUE_UNKNOWN_PART => cstr!("Unknown URL part").as_ptr(),
        CURLUE_NO_SCHEME => cstr!("No scheme part").as_ptr(),
        CURLUE_NO_USER => cstr!("No user part").as_ptr(),
        CURLUE_NO_PASSWORD => cstr!("No password part").as_ptr(),
        CURLUE_NO_OPTIONS => cstr!("No options part").as_ptr(),
        CURLUE_NO_HOST => cstr!("No host part").as_ptr(),
        CURLUE_NO_PORT => cstr!("No port part").as_ptr(),
        CURLUE_NO_QUERY => cstr!("No query part").as_ptr(),
        CURLUE_NO_FRAGMENT => cstr!("No fragment part").as_ptr(),
        CURLUE_NO_ZONEID => cstr!("No zone id part").as_ptr(),
        _ => cstr!("Unknown URL error").as_ptr(),
    }
}
